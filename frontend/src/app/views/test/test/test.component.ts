...
export class TestComponent implements OnInit {
  ...
  actionTestType = ActionTestType;              //сделали такую переменную так как в шаблоне нельзя вставить ActionTestType.prev (1:22 урока)
        ...
        this.testService.getQuiz(params['id'])
          .subscribe(result => {
            if ((result as DefaultResponseType).error !== undefined) {
              throw new Error((result as DefaultResponseType).message);   //также можно использовать снекбар или редирект
            }
