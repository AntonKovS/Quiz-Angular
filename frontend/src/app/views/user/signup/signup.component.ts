                ...
                .subscribe({
                    next: (data: SignupResponseType) => {
                        if (data.error || !data.user) {
                            this._snackBar.open('Ошибка при регистрации');   //что показать решаем сами, можно data.message
                                ...
                                .subscribe({
                                    next: (data: LoginResponseType) => {
                                        if (data.error || !data.accessToken || !data.refreshToken || !data.fullName || !data.userId) {
                                            this._snackBar.open('Ошибка при авторизации');   //что показать решаем сами, можно data.message
